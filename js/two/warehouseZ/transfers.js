        ...
        $(".agains").click(function() {
           ...
           //监听头部监听
            table.on('toolbar(testdome)', function(obj) {
                ...
                switch (obj.event) {
                    case 'add':
                            window.location.href = "./newpage/addtransfers.html";
                        break;
                    case 'update':
                        if(data.length === 0){
                            layer.msg('请选择一行');
                            ...
                            window.location.href = "./newpage/transfersShow.html?id="+data[0].id;
                        }
                        break;
                    case 'delete':
                        if (data.length === 0) {
          ...
          function getList(){
              ...
              var allocateName = document.querySelector('#allocateName').value;
              var ordercode = document.querySelector('#ordercode').value;
            //获取列表
            var tableIns = table.render({
                elem: "#test",
                url: baseaip + "plant/StorageManage/getPlantStorageAllocates",
                method: "GET",
                where: {
                    startDate: startDate,
                    endDate: endDate,
